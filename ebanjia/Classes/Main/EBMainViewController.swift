...
class EBMainViewController: UITabBarController {
    ...
    private func setupUI() {
        ...
        naviHome.tabBarItem.image = UIImage(named:"order_1")
        naviHome.tabBarItem.selectedImage = UIImage(named:"order")
        
        let  orderListVC  = EBOrderListViewController()
        let naviOrderList = UINavigationController(rootViewController: orderListVC)
        naviOrderList.navigationBar.barTintColor = UIColor(red: 0, green: 144/255.0, blue: 227/255.0, alpha: 1)
        naviOrderList.navigationBar.titleTextAttributes = dict as? [NSAttributedStringKey : Any]
        naviOrderList.tabBarItem.title = "我的订单"
        naviOrderList.tabBarItem.image = UIImage(named:"form_1")
        naviOrderList.tabBarItem.selectedImage = UIImage(named:"form")
